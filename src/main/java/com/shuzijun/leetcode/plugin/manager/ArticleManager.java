...
public class ArticleManager {
  ...
  public static void openArticle(Question question, Project project) {
    ...
    String filePath = PersistentConfig.getInstance().getTempFilePath() + Constant.DOC_SOLUTION
      + question.getArticleSlug() + "." + PluginConstant.LEETCODE_EDITOR_VIEW;
    ...
    if (!file.exists()) {
      ...
      if (URLUtils.isCn()) {
        ...
        host =
          URLUtils.getLeetcodeProblems() + question.getTitleSlug() + "/solution/" + question.getArticleSlug() + "/";
      } else {
  ...
  private static String getEnArticle(Question question, Project project) {
    try {
      ...
      httpRequest.setBody("{\"operationName\":\"QuestionNote\",\"variables\":{\"titleSlug\":\""
        + question.getTitleSlug() + "\"},\"query\":\"query QuestionNote($titleSlug: String!) "
      ...
      if (response.getStatusCode() == 200) {
        String content = JSONObject.parseObject(response.getBody()).getJSONObject("data").getJSONObject("question")
          .getJSONObject("solution").getString("content");
  ...
  private static String getCnArticle(Question question, Project project) {
    try {
      HttpRequest httpRequest = HttpRequest.post(URLUtils.getLeetcodeGraphql(), "application/json");
      httpRequest.setBody("{\"operationName\":\"solutionDetailArticle\",\"variables\":{\"slug\":\""
        + question.getArticleSlug() + "\",\"orderBy\":\"DEFAULT\"},"
      ...
      HttpResponse response = HttpRequestUtils.executePost(httpRequest);
      if (response.getStatusCode() == 200) {
        String content = JSONObject.parseObject(response.getBody()).getJSONObject("data")
          .getJSONObject("solutionArticle").getString("content");
        if (StringUtils.isBlank(content)) {
          MessageUtils.getInstance(project).showWarnMsg("error", PropertiesUtils.getInfo("request.auth"));
          return null;
        } else {
  ...
  public static List<Solution> getSolutionList(Question question, Project project) {
    ...
    try {
      HttpRequest httpRequest = HttpRequest.post(URLUtils.getLeetcodeGraphql(), "application/json");
      httpRequest.setBody("{\"operationName\":\"questionSolutionArticles\",\"variables\":{\"questionSlug\":\""
        + question.getTitleSlug() + "\",\"first\":10,\"skip\":0,\"orderBy\":\"DEFAULT\"},\"query\":\"query "
      ...
      HttpResponse response = HttpRequestUtils.executePost(httpRequest);
      if (response.getStatusCode() == 200) {
        JSONArray edges = JSONObject.parseObject(response.getBody()).getJSONObject("data")
          .getJSONObject("questionSolutionArticles").getJSONArray("edges");
