...
public class CodeManager {
  ...
  public static void openContent(Question question, Project project, boolean isOpen) {
    ...
    String filePath = PersistentConfig.getInstance().getTempFilePath() + Constant.DOC_CONTENT
      + VelocityUtils.convert(config.getCustomFileName(), question) + ".md";
  ...
  public static void SubmitCode(Question question, Project project) {
    ...
    try {
      HttpRequest httpRequest =
        HttpRequest.post(URLUtils.getLeetcodeProblems() + question.getTitleSlug() + "/submit/", "application/json");
      ...
      if (response != null && response.getStatusCode() == 200) {
        ...
        LogUtils.LOG
          .error("提交失败：url：" + httpRequest.getUrl() + ";param:" + arg.toJSONString() + ";body:" + response.getBody());
  ...
  public static void RunCodeCode(Question question, Project project) {
    ...
    try {
      HttpRequest httpRequest = HttpRequest
        .post(URLUtils.getLeetcodeProblems() + question.getTitleSlug() + "/interpret_solution/", "application/json");
      JSONObject arg = new JSONObject();
      arg.put("question_id", question.getQuestionId());
            ...
            if ("SUCCESS".equals(jsonObject.getString("state"))) {
              if (jsonObject.getBoolean("run_success")) {
                if (Integer.valueOf(10).equals(jsonObject.getInteger("status_code"))) {
                  ...
                  String runtimePercentile =
                    jsonObject.getBigDecimal("runtime_percentile").setScale(2, BigDecimal.ROUND_HALF_UP).toString();
                  String memory = jsonObject.getString("status_memory");
                  String memoryPercentile =
                    jsonObject.getBigDecimal("memory_percentile").setScale(2, BigDecimal.ROUND_HALF_UP).toString();

                  MessageUtils.getInstance(project).showInfoMsg("info",
                    PropertiesUtils.getInfo("submit.success", runtime, runtimePercentile, codeTypeEnum.getType(),
                      memory, memoryPercentile, codeTypeEnum.getType()));
                  question.setStatus("ac");
                  project.getMessageBus().syncPublisher(QuestionStatusListener.QUESTION_STATUS_TOPIC)
                  ...
                  MessageUtils.getInstance(project).showInfoMsg("info",
                    PropertiesUtils.getInfo("submit.failed", input, output, expected, outputs));
                ...
                String outputs = jsonObject.getString("std_output");
                MessageUtils.getInstance(project).showInfoMsg("info", PropertiesUtils.getInfo("submit.run.failed",
                  buildErrorMsg(jsonObject), jsonObject.getString("last_testcase"), outputs));
                if (!"ac".equals(question.getStatus())) {
                  question.setStatus("notac");
                  project.getMessageBus().syncPublisher(QuestionStatusListener.QUESTION_STATUS_TOPIC)
                    .updateTable(question);
            ...
            if ("SUCCESS".equals(jsonObject.getString("state"))) {
              if (!key.equals(returnObj.getString("interpret_id"))) {
                ...
                if (jsonObject.getBoolean("run_success")) {
                  ...
                  String expected = "";
                  if (returnObj.getJSONArray("expected_code_answer") != null
                    && !returnObj.getJSONArray("expected_code_answer").isEmpty()) {
                    expected = Joiner.on("\n").join(returnObj.getJSONArray("expected_code_answer"));
                  } else if (jsonObject.getJSONArray("expected_code_answer") != null
                    && !jsonObject.getJSONArray("expected_code_answer").isEmpty()) {
                    expected = Joiner.on("\n").join(jsonObject.getJSONArray("expected_code_answer"));
                  }
                  String outputs = StringUtils.join(jsonObject.getJSONArray("code_output"), "\n\t\t");
                  MessageUtils.getInstance(project).showInfoMsg("info",
                    PropertiesUtils.getInfo("test.success", input, output, expected, outputs));
                } else {
                  String outputs = StringUtils.join(jsonObject.getJSONArray("code_output"), "\n\t\t");
                  MessageUtils.getInstance(project).showInfoMsg("info",
                    PropertiesUtils.getInfo("submit.run.failed", buildErrorMsg(jsonObject), input, outputs));
                }
                return;
              }
            }
