...
public class ArticleServiceImpl implements ArticleService {
    ...
    @Override
    public File openArticle(String titleSlug, String articleSlug, Boolean isOpenEditor) {
        ...
        String host;
        if (!file.exists()) {
            String article = getArticle(titleSlug, articleSlug);
            if (URLUtils.isCn()) {
    ...
    @Override
    public String getArticle(String titleSlug, String articleSlug) {
        try {
            HttpResponse response = Graphql.builder(repositoryService).cn(URLUtils.isCn()).operationName("solutionDetailArticle").variables("slug", articleSlug).variables("titleSlug", titleSlug).variables("orderBy", "DEFAULT").request();
    ...
    @Override
    public String formatMarkdown(String content, String host) {
        return CleanMarkdown.cleanMarkdown(content, host);
    }

    @Override
    public List<Solution> getSolutionList(String titleSlug) {
        List<Solution> solutionList = new ArrayList<>();

        try {
            HttpResponse response = Graphql.builder(repositoryService).cn(URLUtils.isCn()).operationName("questionSolutionArticles").variables("questionSlug", titleSlug).variables("first", 200).variables("skip", 0).variables("orderBy", "DEFAULT").request();
            if (response.getStatusCode() == 200) {
