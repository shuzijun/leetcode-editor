...
public class FindServiceImpl implements FindService {
    ...
    public List<Tag> getTags() {
        ...
        HttpResponse response = repositoryService.HttpRequest().get(URLUtils.getLeetcodeTags()).cache(true).request();
    ...
    public List<Tag> getLists() {
        ...
        HttpResponse response = repositoryService.HttpRequest().get(URLUtils.getLeetcodeFavorites()).cacheParam(WindowFactory.getDataContext(project).getData(DataKeys.LEETCODE_PROJECTS_TABS).getUser().getUsername()).request();
        if (response.getStatusCode() == 200) {
            try {
                String body = response.getBody();
                tags = parseList(body);
    ...
    public List<Tag> getCategory() {
        ...
        HttpResponse response = repositoryService.HttpRequest().get(URLUtils.getLeetcodeCardInfo()).cache(true).request();
        if (response.getStatusCode() == 200) {
            try {
                String body = response.getBody();
    ...
    private List<Tag> parseTag(String str) {
    ...
    private List<Tag> parseCategory(String str) {
        List<Tag> tags = new ArrayList<Tag>();
    ...
    private List<Tag> parseList(String str) {
        List<Tag> tags = new ArrayList<Tag>();
