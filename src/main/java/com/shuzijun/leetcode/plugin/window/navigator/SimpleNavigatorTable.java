 ...
 * @author shuzijun
 */
public class SimpleNavigatorTable extends NavigatorTableData<QuestionView> {


    private NavigatorAction myNavigatorAction;

    public SimpleNavigatorTable(Project project, NavigatorAction navigatorAction) {
        super(project);
    ...
    protected MyTableModel<QuestionView> createMyTableModel() {
        return new MyTableModel<QuestionView>(new String[]{"Status", "Title", "Acceptance", "Difficulty", "Frequency"}, new String[]{"S", "Title", "AC", "DD", "F"}) {
            ...
            public Object getValue(QuestionView question, int columnIndex) {
                ...
                if (columnIndex == 2) {
                    return formatPercentage(nf, question.getAcceptance());
                }
                ...
                if (columnIndex == 4) {
                    return formatPercentage(nf, question.getFrequency());
                }
                return null;
            }
        };
    ...
    static String formatPercentage(NumberFormat numberFormat, Number value) {
    ...
    protected void setColumnWidth(MyJBTable myJBTable) {
        ...
        if (showFrequencyColumn(URLUtils.getLeetcodeHost())) {
            myJBTable.getColumnModel().getColumn(4).setMaxWidth(60);
        } else if (myJBTable.getColumnModel().getColumnCount() > 4) {
            myJBTable.removeColumn(myJBTable.getColumnModel().getColumn(4));
        }
    }

    static boolean showFrequencyColumn(String host) {
        return URLUtils.leetcodecn.equalsIgnoreCase(host);
    }
