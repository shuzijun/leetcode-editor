...
public class FileUtils {
  ...
  public static String getClearCommentFileBody(File file, CodeTypeEnum codeTypeEnum) {
    ...
    try {
      String body = ApplicationManager.getApplication()
        .runReadAction((Computable<String>)() -> FileDocumentManager.getInstance().getDocument(vf).getText());
      if (StringUtils.isNotBlank(body)) {
        ...
        for (String line : lines) {
          if (StringUtils.isNotBlank(line)
            && trim(line).equals(trim(codeTypeEnum.getComment() + Constant.SUBMIT_REGION_BEGIN))) {
            codeBegins.add(lineCount);
          } else if (StringUtils.isNotBlank(line)
            && trim(line).equals(trim(codeTypeEnum.getComment() + Constant.SUBMIT_REGION_END))) {
  ...
  public static void openFileEditorAndSaveState(File file, Project project, Question question,
    BiConsumer<LeetcodeEditor, String> consumer, boolean isOpen) {
    ApplicationManager.getApplication().invokeAndWait(() -> {
      VirtualFile vf = LocalFileSystem.getInstance().refreshAndFindFileByIoFile(file);
      LeetcodeEditor leetcodeEditor =
        ProjectConfig.getInstance(project).getDefEditor(URLUtils.getLeetcodeHost() + question.getFrontendQuestionId());
